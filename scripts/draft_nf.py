...
7. SOCIAL COPY. Write all five captions (LinkedIn primary, Instagram Reels, TikTok, YouTube Shorts with title+description, Facebook), each platform-shaped per the example, following the CAPTION + HASHTAG STRATEGY doc above: hashtag caps are 3-5 on LinkedIn / Instagram / TikTok / YouTube, 2-3 on Facebook (realtor-first, drop the long tail). Do NOT write an X/Twitter caption -- D.J. does not use X. Front-load one real in-app-search keyword (the plain phrase an agent would actually search, e.g. "real estate commission settlement", "Chicago real estate") into the first line or two of each caption. Optimize for the send (would one agent DM this to another) and for dwell, never an engagement ask.
8. AI MUSIC PROMPT. Include the section per the presets: Vibe and a single CapCut AI Music prompt (<=300 chars, starts with the no-vocals constraint, includes BPM as a number). D.J. edits only in CapCut, so there is no Suno/Udio prompt. Tune BPM/energy to this story.
