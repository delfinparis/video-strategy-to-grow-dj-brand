---
series: "Inside the Industry"
type: "reactive"
script_number: "NF-047"
title: "When eXp Talks Buyer Harm, It's Not a Blog Post"
avatar: "All -- brokers and any agent running coming-soon or office-exclusive inventory"
content_pillar: "market_intelligence"
post_date: "TBD"
status: "draft"
time_sensitive: "High -- Inman published the Lysenko piece 2026-06-16. Post within 3-4 days while the quote is still circulating."
shareable_moment: "When a major brokerage puts its name on that side publicly, it isn't a blog post. It's a legal positioning move."
length_justification: "Reactive piece with the named executive, the direct quote, the strategic read, and a concrete audit action. Each beat earns its place."
companion_to: "NF-027 (Compass billboard), NF-035 (NY AG antitrust), NF-038 (NAR Policy 8.5), NF-040 (MLS neutrality), NF-046 (CT private-listings law, posted same week). NF-047 is the major-brokerage chapter of the private-listings arc. Space at least 4-5 days from NF-046 so the theme doesn't stack."
---

# When eXp Talks Buyer Harm, It's Not a Blog Post

> **WOW: Earned inside-the-industry observation -- the CTO of eXp Realty, formerly CEO of Zoocasa, just put her name on the buyer-harm argument against off-MLS listings. When a major brokerage takes that position publicly, it isn't thought leadership. It's a legal positioning move that aligns eXp with the lawsuits, the state laws, and the DOJ at the exact moment private listings are getting relitigated.**

**Pillar:** Market Intelligence | **Avatars:** All, brokers and any agent running coming-soon or office-exclusive inventory
**Filming:** Walk and talk. No demo.

## Shareable Moment
> "When a major brokerage takes that side publicly, it isn't a blog post. It's a legal positioning move."

## Script (~68 seconds, walk and talk)

### HOOK (0:00-0:10)
eXp's tech chief just made the buyer-harm argument against private listings. When a major brokerage takes that side publicly, it isn't a blog post. It's a legal positioning move.

### CONTEXT (0:10-0:28)
Carrie Lysenko runs tech at eXp. She used to be CEO of Zoocasa. The portal. Her quote, today. "The person bringing the money to the transaction keeps getting left out of the off-MLS conversation. That should bother everyone." She means the buyer.

### WHY IT MATTERS (0:28-0:46)
This isn't a thought-leadership beat. eXp just publicly aligned with the side suing Compass, the side writing the state laws, the side at the DOJ. Anyone running a private-listing program just got a well-funded opponent willing to make the consumer-harm case in court.

### CLOSE (0:46-1:06)
Here's what you do now. Audit your coming-soon and office-exclusive practices against your brokerage's written policy today. Not next week. The buyer-harm argument is getting louder. The next person making it might be a regulator or a plaintiff's firm.

**Estimated Duration:** ~68 seconds | **Word Count:** ~170 words

## Data Source

- **Claim:** "eXp's tech chief just made the buyer-harm argument against private listings... Carrie Lysenko runs tech at eXp. She used to be CEO of Zoocasa."
  - Source: Inman, "EXp Tech Chief: The Off-MLS Debate Keeps Leaving Out Buyers," 2026-06-16 ([link](https://www.inman.com/2026/06/16/exp-cto-off-mls-debate-home-buyers/)). Carrie Lysenko, CTO of eXp Realty and former CEO of Zoocasa, published an op-ed/interview making the buyer-harm argument in the off-MLS debate.
  - Status: confirmed

- **Claim (direct quote):** "The person bringing the money to the transaction keeps getting left out of the off-MLS conversation. That should bother everyone."
  - Source: Inman 2026-06-16 (above), quoting Lysenko directly.
  - Status: confirmed (direct quote)

- **Claim:** "eXp just publicly aligned with the side suing Compass, the side writing the state laws, the side at the DOJ."
  - Source: Industry framing across coverage. Companies and entities currently opposing Compass's private-listings model include: Zillow (lawsuit filed 2026-05-12 against Compass and MRED); state governments (Connecticut SB 340 signed 2026-05-27, NY bill on Hochul's desk); the DOJ (longstanding antitrust scrutiny of private-listing networks and Clear Cooperation Policy implementation). eXp's public position via Lysenko and prior CEO Leo Pareja statements supporting Clear Cooperation aligns eXp with that coalition.
  - Status: confirmed (factual alignment of public positions)

- **Claim:** "Anyone running a private-listing program just got a well-funded opponent willing to make the consumer-harm case in court."
  - Source: D.J. Paris's strategic read. eXp's public buyer-harm framing is the predicate consumer-harm argument that plaintiffs' counsel and state AGs use in antitrust and consumer-protection cases. The "well-funded" reference is to eXp's size (one of the largest U.S. brokerages by agent count); the "in court" reference is to the litigation/regulatory exposure agents and brokerages with private-listing or coming-soon programs now face.
  - Status: editorial framing, not a stat

- **Series / overlap notes:**
  - Builds the private-listings arc: NF-027 (consumer angle), NF-035 (NY AG antitrust), NF-038 (NAR Policy 8.5), NF-040 (MLS neutrality), NF-046 (CT law signed).
  - NF-047 is the major-brokerage-coalition chapter -- it shows the industry split visible at the brokerage level.
  - Space at least 4-5 days from NF-046 (CT law) so the private-listings theme doesn't run consecutively.

## AI Music Prompt

**Vibe:** Default NF news-explainer with the strategic-read edge. The hook lands on a corporate-positioning reveal, the body walks through the legal-coalition alignment. Bed should feel authoritative and slightly tense, Bloomberg-explainer adjacent.

**Suno / Udio:**
> [no vocals] modern news-explainer underscore, instrumental only, urgent but controlled, 95 BPM, pulsing minor-key synth pad, muted plucked synth motif, tight kick, subtle hi-hat shuffle, light string swell on the back half, clean low end, unobtrusive background for voiceover, no melodic hook, no vocal chops, no risers, no whooshes, loopable

**CapCut AI Music:**
> Instrumental news-explainer underscore, urgent but controlled, 95 BPM, pulsing minor synth pad, muted plucks, tight kick, no vocals, subtle bed for voiceover, loopable

## Social Copy

### LinkedIn (PRIMARY)
**Caption:**
eXp's tech chief just made the buyer-harm argument against private listings. When a major brokerage puts its name on that side publicly, it isn't a blog post. It's a legal positioning move.

Carrie Lysenko is CTO of eXp Realty. Before eXp, she was CEO of Zoocasa, the consumer portal. She has run both sides of the glass.

Her statement, published in Inman today:

"The person bringing the money to the transaction keeps getting left out of the off-MLS conversation. That should bother everyone."

She means the buyer.

Here is the read.

This isn't thought leadership. It is a legal and regulatory positioning move. eXp just publicly aligned with the side suing Compass, the side writing the state laws (Connecticut signed last month, New York is on the governor's desk), and the side at the DOJ.

The "buyers are harmed by off-MLS" framing is the exact predicate argument plaintiffs' counsel and state attorneys general use in antitrust and consumer-protection cases.

Anyone running a private-listing program, a coming-soon pocket strategy, or an office-exclusive network just got a well-funded opponent willing to make that case publicly. And in court.

Audit your coming-soon and office-exclusive practices against your brokerage's written policy today. Not next week. The buyer-harm argument is getting louder. The next person making it might be a regulator or a plaintiff's firm, and you do not want them looking at your last six months of listings.

**Hashtags:** #RealEstate #eXpRealty #Compass #PrivateListings #ClearCooperation #RealEstateLaw #BrokerageLaw #RealtorLife #KeepingItRealPodcast #InsideTheIndustry

### Instagram Reels
**Caption:**
eXp's tech chief just made the buyer-harm argument against private listings.

When a major brokerage puts its name on that side publicly, it isn't a blog post. It's a legal positioning move.

Carrie Lysenko, CTO of eXp, former CEO of Zoocasa, said today: "The person bringing the money to the transaction keeps getting left out of the off-MLS conversation."

eXp just publicly aligned with the side suing Compass, the side writing the state laws, the side at the DOJ.

If you run a private-listing or coming-soon program, you just got a well-funded opponent willing to make the consumer-harm case.

Audit your coming-soon practices against your brokerage's written policy today. Not next week.

**Hashtags:** #realestate #exprealty #compass #privatelistings #clearcooperation #realestatelaw #brokeragelaw #realtorlife #keepingitrealpodcast #insidetheindustry #realestatenews

### TikTok
**Caption:** eXp's tech chief just made the buyer-harm case against private listings. When a major brokerage takes that side publicly, it isn't a blog post. It's legal positioning.
**Hashtags:** #realtortok #realestatetok #realtor #compass #exprealty #realestatenews

### YouTube Shorts
**Title:** When eXp Talks Buyer Harm, It's Not a Blog Post
**Description:** Carrie Lysenko, CTO of eXp Realty and former CEO of consumer portal Zoocasa, just published a public statement saying buyers are getting left out of the off-MLS conversation. "The person bringing the money to the transaction keeps getting left out... That should bother everyone." D.J. Paris on why a major brokerage taking that position publicly is not a thought-leadership piece — it's a legal positioning move that aligns eXp with the lawsuits, the state laws, and the DOJ at the exact moment private listings are getting relitigated. And the one audit every brokerage and agent should run today.
**Hashtags:** #shorts #realestate #exprealty #compass #privatelistings #realestatelaw #keepingitrealpodcast #insidetheindustry

### Facebook
**Caption:**
eXp's tech chief just made the buyer-harm argument against private listings. When a major brokerage puts its name on that side publicly, it isn't a blog post. It's a legal positioning move.

Carrie Lysenko is CTO of eXp Realty and former CEO of consumer portal Zoocasa. Her statement, published in Inman today: "The person bringing the money to the transaction keeps getting left out of the off-MLS conversation. That should bother everyone." She means the buyer.

This isn't thought leadership. eXp just publicly aligned with the side suing Compass, the side writing the state laws (Connecticut signed last month, New York is on the governor's desk), and the side at the DOJ.

The "buyers are harmed by off-MLS" framing is the exact predicate argument plaintiffs' counsel and state attorneys general use in antitrust and consumer-protection cases. Anyone running a private-listing program, a coming-soon pocket, or an office-exclusive network just got a well-funded opponent willing to make the case in court.

Audit your coming-soon and office-exclusive practices against your brokerage's written policy today. Not next week.

**Hashtags:** #RealEstate #eXpRealty #Compass #PrivateListings #ClearCooperation #RealEstateLaw #RealtorLife

### X (Twitter)
**Caption:**
eXp's tech chief just made the buyer-harm case against private listings.

When a major brokerage puts its name on that side publicly, it isn't a blog post. It's a legal positioning move.

eXp just aligned with the side suing Compass, the side writing the state laws, the side at the DOJ.

Audit your coming-soon practices today.

**Hashtags:** #RealEstate #eXp #PrivateListings #ClearCooperation
