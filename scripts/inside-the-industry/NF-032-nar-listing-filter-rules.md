---
series: "Inside the Industry"
type: "reactive"
script_number: "NF-032"
title: "NAR Just Published Rules on Who Controls Your Listings"
avatar: "All -- brokers and listing-focused agents especially"
content_pillar: "market_intelligence"
post_date: "TBD"
status: "draft"
time_sensitive: "High -- NAR resources published 2026-05-29 in direct response to the MRED/Compass/Zillow fight. Post within 5 days of release while the news cycle is live."
companion_to: "NF-027 (Compass billboard / private listings -- consumer-facing seller-call angle). NF-032 is the broker/MLS-rules angle on the same dispute. Space them several days apart."
shareable_moment: "The private-exclusive playbook just got narrower."
---

# NAR Just Published Rules on Who Controls Your Listings

> **WOW: Earned inside-the-industry observation + tactical specificity -- two days after the federal lawsuit naming MRED, NAR published three resources clarifying Policy 8.5 and the Clear Cooperation Policy. The script names the exact allowed and prohibited filtering categories agents should test their own MLS against, and the specific check to run on IDX and VOW rules this week.**

**Pillar:** Market Intelligence | **Avatars:** All, brokers and listing-focused agents especially
**Filming:** Walk and talk. No demo.

## Shareable Moment
> "The private-exclusive playbook just got narrower."

## Script (~66 seconds, walk and talk)

### HOOK (0:00-0:09)
NAR just spelled out the rules for how your MLS can display and filter listings. Two days ago. Most agents haven't read them.

### CONTEXT (0:09-0:31)
Three guides clarifying Policy 8.5. MLSs can filter listings by objective criteria. Geography. Price. Property type. They cannot filter by agent or brokerage name. They cannot filter by compensation. And clear cooperation got a sharper line on when off-MLS broker conversations count as public marketing.

### WHY IT MATTERS (0:31-0:48)
This isn't NAR housekeeping. If your MLS uses any of those filters, your MLS just got told to stop. The private-exclusive playbook also got narrower. The moment a broker-to-broker conversation crosses into marketing, the listing has to be in the MLS within twenty-four hours.

### CLOSE (0:46-1:06)
Here's what you do now. This week, pull your MLS's IDX and VOW rules. Check two things. Whether users can filter by agent or brokerage name. And whether they can filter by compensation. If either, flag it to leadership. And if you've been running private exclusives, read the clear cooperation clarifier.

**Estimated Duration:** ~66 seconds | **Word Count:** ~165 words

## Data Source

- **Claim:** "NAR just published the rules... Two days ago"
  - Source: Real Estate News, "NAR listing guides aim to promote 'clear understanding' of intent," 2026-05-29 ([link](https://www.realestatenews.com/2026/05/29/nar-listing-guides-aim-to-promote-clear-understanding-of-intent)); NAR REALTOR Magazine, "NAR Publishes Guidance on the Use of Listing Filters, Clear Cooperation" ([link](https://www.nar.realtor/magazine/real-estate-news/nar-publishes-guidance-on-the-use-of-listing-filters-clear-cooperation)). NAR released three resources on 2026-05-29.
  - Status: confirmed
  - Note: "Two days ago" is accurate if posted on or around 2026-05-31. Update or generalize to "this week" if posted later.

- **Claim:** "Three guides clarifying Policy 8.5"
  - Source: Real Estate News 2026-05-29. NAR released three resources, two addressing listing filtering in IDX and VOW displays (operating in tandem with MLS Policy 8.5), and a third addressing the Clear Cooperation Policy.
  - Status: confirmed

- **Claim:** "MLSs can filter listings by objective stuff. Geography. Price. Property type."
  - Source: Real Estate News 2026-05-29. NAR's examples of objective criteria include geography, list price range, and property type, applied equally to all participants based on measurable facts.
  - Status: confirmed (NAR examples, not an exhaustive list)

- **Claim:** "They cannot filter by agent name, brokerage name, or compensation"
  - Source: Real Estate News 2026-05-29 and NAR MLS Policy 8.5. NAR policy prohibits MLSs from allowing users to filter listings based on offers of compensation or the name of the brokerage or agent representing the listing. Objective criteria must "not explicitly and/or directly target any particular brokerage and/or agent by name."
  - Status: confirmed

- **Claim:** "Clear cooperation got a sharper line on when off-MLS broker conversations count as public marketing... within twenty-four hours"
  - Source: Real Estate News 2026-05-29. The third NAR resource explains when a one-to-one broker-to-broker communication would and would not trigger NAR's Clear Cooperation Policy. CCP requires listings to be entered into the MLS within one business day (commonly framed as 24 hours) of being publicly marketed.
  - Status: confirmed
  - Note: NAR's clarifier acknowledges some one-to-one broker communications do NOT trigger CCP. The script captures the agent-relevant edge ("the moment it crosses into marketing") without overclaiming a blanket prohibition.

- **Claim:** "Direct response to the federal lawsuit naming Chicago's MLS"
  - Source: Real Estate News 2026-05-29 confirms the resources are "timely" given the recent disputes. Compass CEO Robert Reffkin cited NAR's MLS Policy 8.5 when challenging Zillow's standards as "subjective." MLS/CLAW subsequently updated its IDX policy in apparent response.
  - Series continuity: NF-027 covered the consumer-facing side of this dispute (Compass billboards, Chicago Zillow listings going dark, Judge Tharp's May 22 restore order). NF-032 is the regulatory follow-up. Space the two posts at least several days apart so the topic does not feel repetitive on the feed.
  - Status: confirmed

- **Editorial note -- single staccato triple by design:** The CONTEXT beat keeps one staccato triple ("Geography. Price. Property type.") for the allowed criteria and splits the prohibited side into two sentences ("They cannot filter by agent or brokerage name. They cannot filter by compensation.") so the script doesn't carry two close-quarters rule-of-three lists. Both regulatory categories are still enumerated; only the structure is balanced.

## AI Music Prompt

**Vibe:** Default NF news-explainer. Sober, modern, slightly tense. Bloomberg / WSJ explainer energy. The hook is regulatory news and the body is a compliance brief, so the bed should feel authoritative without being dramatic.

**Suno / Udio:**
> [no vocals] modern news-explainer underscore, instrumental only, urgent but controlled, 95 BPM, pulsing minor-key synth pad, muted plucked synth motif, tight kick, subtle hi-hat shuffle, light string swell on the back half, clean low end, unobtrusive background for voiceover, no melodic hook, no vocal chops, no risers, no whooshes, loopable

**CapCut AI Music:**
> Instrumental news-explainer underscore, urgent but controlled, 95 BPM, pulsing minor synth pad, muted plucks, tight kick, no vocals, subtle bed for voiceover, loopable

## Social Copy

### LinkedIn (PRIMARY)
**Caption:**
NAR just spelled out the rules for how your MLS can display and filter listings. Two days ago. Most agents have not read them.

Three resources clarifying Policy 8.5 and the Clear Cooperation Policy. The short version.

MLSs can filter listings by objective criteria. Geography. Price. Property type. The measurable stuff.

MLSs cannot filter by agent name, brokerage name, or compensation offers.

And the Clear Cooperation Policy got a sharper line on when a one-to-one broker-to-broker conversation crosses into public marketing. The moment it does, the listing has to be in the MLS within 24 hours.

This is not NAR housekeeping. The guidance landed two days after the federal lawsuit naming Chicago's MLS. If your MLS is currently filtering listings by any of those prohibited filters, it just got told to stop. And the private-exclusive playbook quietly got narrower.

This week, pull your MLS's IDX and VOW rules. Check whether users can filter by agent or brokerage name, and whether they can filter by compensation. If either, flag it to leadership.

If you have been running private exclusives, read NAR's clear cooperation clarifier before you do the next one.

**Hashtags:** #RealEstate #NAR #MLS #RealEstateLaw #ListingStrategy #ChicagoRealEstate #BrokerageLaw #RealtorLife #KeepingItRealPodcast #InsideTheIndustry

### Instagram Reels
**Caption:**
NAR just spelled out the rules for how your MLS can display and filter listings. Two days ago. Most agents haven't read them.

Three guides clarifying Policy 8.5. MLSs can filter by geography, price, property type. They cannot filter by agent or brokerage name. They cannot filter by compensation. And clear cooperation got a sharper line on when off-MLS broker conversations count as public marketing.

If your MLS filters by any of those names, your MLS just got told to stop. The private-exclusive playbook also got narrower.

Pull your MLS's IDX rules this week. Check whether users can filter by agent or brokerage name, or by compensation. If either, flag it.

**Hashtags:** #realestate #nar #mls #realestatelaw #listingstrategy #chicagorealestate #realtorlife #realestateagent #keepingitrealpodcast #insidetheindustry #realestatenews

### TikTok
**Caption:** NAR just published rules on how your MLS can filter listings. Two days ago. Here's what they say and the exact check to run on your MLS this week.
**Hashtags:** #realtortok #realestatetok #realtor #nar #realestatelaw #realestatenews

### YouTube Shorts
**Title:** NAR Just Published Rules on Who Controls Your Listings
**Description:** Two days after a federal lawsuit named Chicago's MLS in the Compass vs Zillow private-listings fight, NAR released three resources clarifying Policy 8.5 and the Clear Cooperation Policy. The short version: MLSs can filter listings by objective criteria like geography, price, and property type, but cannot filter by agent name, brokerage name, or compensation offers. And one-to-one broker conversations now have a sharper line on when they cross into public marketing. D.J. Paris on the exact check to run on your MLS's IDX and VOW rules this week.
**Hashtags:** #shorts #realestate #nar #mls #realestatelaw #realestatenews #keepingitrealpodcast #insidetheindustry

### Facebook
**Caption:**
NAR just spelled out the rules for how your MLS can display and filter listings. Two days ago. Most agents haven't read them.

Three guides clarifying Policy 8.5. MLSs can filter listings by objective criteria. Geography. Price. Property type. They cannot filter by agent name, brokerage name, or compensation offers. And the Clear Cooperation Policy got a sharper line on when off-MLS broker conversations count as public marketing.

This isn't NAR housekeeping. The guidance landed two days after the federal lawsuit naming Chicago's MLS. If your MLS filters by any of those prohibited filters, it just got told to stop. The private-exclusive playbook also got narrower.

This week, pull your MLS's IDX and VOW rules. Check whether users can filter by agent or brokerage name, and whether they can filter by compensation. If either, flag it to leadership.

If you've been running private exclusives, read NAR's clear cooperation clarifier before you do the next one.

**Hashtags:** #RealEstate #NAR #MLS #RealEstateLaw #ChicagoRealEstate #RealtorLife

### X (Twitter)
**Caption:**
NAR just published rules for how your MLS can display and filter listings. 2 days ago.

Allowed: geography, price, property type. The objective stuff.
Banned: agent name, brokerage name, compensation offers.

And clear cooperation got a sharper line on when broker-to-broker conversations turn into public marketing.

The private-exclusive playbook just got narrower.

**Hashtags:** #RealEstate #NAR #MLS #RealEstateLaw
